...
const balances = new Map();     // nickname -> total wallet balance
...
let auction = {
  ...
  holds: new Map(), // nickname -> held amount during current auction
  logs: []
};
...
function participantList(){
  return [...participants.entries()]
    .map(([id,u])=>({id,name:u.name,joinedAt:u.joinedAt,balance:balances.get(u.name)||0,held:auction.holds.get(u.name)||0}))
...
function sendAdminState(){
  ...
  for(const id of adminSockets){ io.to(id).emit('admin_state', payload); }
}
function broadcastAll(){ broadcastParticipants(); broadcastState(); sendAdminState(); }
function isAdmin(socket){ return adminSockets.has(socket.id); }
function available(name){ return (balances.get(name)||0) - (auction.holds.get(name)||0); }
...
io.on('connection', socket=>{
  ...
  socket.on('admin_set_balance', (payload={}, callback=()=>{})=>{
    ...
    if(!name || amount === null) return callback({ok:false,message:'닉네임과 금액을 확인해주세요.'});
    if(![...participants.values()].some(p=>p.name===name)) return callback({ok:false,message:'현재 접속 중인 참가자가 아닙니다.'});
    const held = auction.holds.get(name)||0;
    if(amount < held) return callback({ok:false,message:`현재 입찰 보류금 ${held.toLocaleString('ko-KR')}원보다 낮게 설정할 수 없습니다.`});
    balances.set(name,amount); // ABSOLUTE set, including 0
  ...
  socket.on('admin_set_start_price', (payload={}, callback=()=>{})=>{
    if(!isAdmin(socket)) return callback({ok:false,message:'방장 권한이 필요합니다.'});
    if(auction.leader) return callback({ok:false,message:'입찰이 시작된 뒤에는 시작가를 변경할 수 없습니다.'});
    const amount = moneyInt(payload.amount);
    if(amount === null) return callback({ok:false,message:'올바른 시작가를 입력해주세요.'});
  ...
  socket.on('place_bid', (payload={}, callback=()=>{})=>{
    ...
    if(auction.status === 'sold') return callback({ok:false,message:'마감된 경매입니다.'});
    const inc = moneyInt(payload.inc);
    if(!inc || inc <= 0) return callback({ok:false,message:'입찰 금액이 올바르지 않습니다.'});

    const nextPrice = auction.currentPrice + inc;
    const myBalance = balances.get(me.name)||0;
    if(nextPrice > myBalance) return callback({ok:false,message:'보유금이 부족합니다.'});

    // 이전 최고입찰자의 보류금은 자동 해제(환불). 새 최고입찰자만 현재 낙찰가를 보류.
    clearHolds();
    auction.holds.set(me.name,nextPrice);
    auction.currentPrice = nextPrice;
    auction.leader = me.name;
    auction.status = 'live';
    const log = {name:me.name,inc,total:nextPrice,ts:Date.now()};
    auction.logs.unshift(log);
    auction.logs = auction.logs.slice(0,100);

    callback({ok:true,balance:myBalance,held:nextPrice,available:myBalance-nextPrice});
    broadcastAll();
  });

  socket.on('admin_close_auction', (payload={}, callback=()=>{})=>{
    if(!isAdmin(socket)) return callback({ok:false,message:'방장 권한이 필요합니다.'});
    if(!auction.leader){
      // 유찰: 모든 보류금 해제. 실제 잔액은 변하지 않음.
      clearHolds();
    ...
    if(bal < price) return callback({ok:false,message:'낙찰자의 보유금이 부족합니다.'});
    balances.set(winner,bal-price); // 낙찰 시에만 실제 차감
    clearHolds();
    auction.status = 'sold';
  ...
  socket.on('admin_cancel_auction', (payload={}, callback=()=>{})=>{
    if(!isAdmin(socket)) return callback({ok:false,message:'방장 권한이 필요합니다.'});
    // 유찰/취소: 보류만 해제, 누구의 실제 잔액도 차감하지 않음.
    clearHolds();
  ...
  socket.on('admin_new_auction', (payload={}, callback=()=>{})=>{
    if(!isAdmin(socket)) return callback({ok:false,message:'방장 권한이 필요합니다.'});
    clearHolds();
    auction.currentPrice = auction.startPrice;
    auction.leader = null;
  ...
  socket.on('disconnect', ()=>{
    ...
    if(u){
      ...
      // 접속 종료가 현재 최고입찰자라면 입찰을 취소하고 보류금 해제.
