...
let auction={title:'오늘의 메인 슬롯',startPrice:0,currentPrice:0,leader:null,status:'ready',holds:new Map(),logs:[]};
...
const state=()=>({title:auction.title,startPrice:auction.startPrice,currentPrice:auction.currentPrice,leader:auction.leader,status:auction.status,logs:auction.logs.slice(0,100)});
...
function all(){io.emit('participants',plist());io.emit('auction_state',state());adminState()}function isA(s){return admins.has(s.id)}function clearH(){auction.holds.clear()}
...
io.on('connection',s=>{
 ...
 s.on('admin_login',(p={},cb=()=>{})=>{if(String(p.password||'')!==ADMIN_PASSWORD)return cb({ok:false,message:'비밀번호가 올바르지 않습니다.'});admins.add(s.id);cb({ok:true,token:crypto.randomBytes(8).toString('hex')});adminState();s.emit('auction_state',state())});
 ...
 s.on('admin_new_auction',(_p={},cb=()=>{})=>{if(!isA(s))return cb({ok:false,message:'방장 권한이 필요합니다.'});clearH();auction={...auction,title:'오늘의 메인 슬롯',startPrice:0,currentPrice:0,leader:null,status:'ready',holds:new Map(),logs:[]};cb({ok:true});all()});
 s.on('admin_end_auction',(_p={},cb=()=>{})=>{if(!isA(s))return cb({ok:false,message:'방장 권한이 필요합니다.'});clearH();auction.status='ended';cb({ok:true});all()});
 s.on('admin_off_auction',(_p={},cb=()=>{})=>{if(!isA(s))return cb({ok:false,message:'방장 권한이 필요합니다.'});participants.clear();balances.clear();clearH();auction={title:'오늘의 메인 슬롯',startPrice:0,currentPrice:0,leader:null,status:'ready',holds:new Map(),logs:[]};cb({ok:true});io.emit('auction_off');all()});
